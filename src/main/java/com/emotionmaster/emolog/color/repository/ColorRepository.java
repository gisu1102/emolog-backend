...
public interface ColorRepository extends JpaRepository<Color, Long> {
    ...
    @Query("SELECT new com.emotionmaster.emolog.color.dto.response.ColorAndDate(c.diary.date, c.hexa) FROM Color c WHERE MONTH(c.diary.date) = :month")
    List<ColorAndDate> findColorAndDateByMonth(int month);

    @Query(value = "SELECT new com.emotionmaster.emolog.color.dto.response.ColorAndDate(c.diary.date, c.hexa) FROM Color c " +
